...
import Lottie
import UIKit
import PullToRefresh_Framework
...
public class ShakuroLogoPullToRefreshContentView: UIView {
    ...
    private var pullingAnimationView: AnimationView?
    private var refreshingAnimationView: AnimationView?
    ...
    private func commonInit() {
        if let jsonFilePath = Bundle.main.path(forResource: Constant.pullingJSONfilename, ofType: nil) {
            let animation = AnimationView(filePath: jsonFilePath)
            animation.currentProgress = 0.0
            animation.translatesAutoresizingMaskIntoConstraints = false
            animation.frame = self.bounds
            self.addSubview(animation)
            if let animationSize = animation.animation?.size {
                animation.widthAnchor.constraint(equalTo: animation.heightAnchor, multiplier: animationSize.width / animationSize.height).isActive = true
            }
        ...
        if let jsonFilePath = Bundle.main.path(forResource: Constant.refreshingJSONfilename, ofType: nil) {
            ...
            self.addSubview(animation)
            if let animationSize = animation.animation?.size {
                animation.widthAnchor.constraint(equalTo: animation.heightAnchor,
                                                 multiplier: animationSize.width / animationSize.height).isActive = true
            }
            animation.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 0.0).isActive = true
            trailingAnchor.constraint(equalTo: animation.trailingAnchor, constant: 0.0).isActive = true
            animation.centerYAnchor.constraint(equalTo: centerYAnchor, constant: 0.0).isActive = true
    ...
    public func length(forWidth: CGFloat) -> CGFloat {
        let result: CGFloat
        if let animationSize = pullingAnimationView?.animation?.size {
